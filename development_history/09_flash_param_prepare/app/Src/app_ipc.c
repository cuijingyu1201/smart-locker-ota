...
void App_IPC_Init(void)
{
    ...
    if (g_irq_sem_handle == NULL) {
        printf("[ERROR] osSemaphoreNew(g_irq_sem) failed!\r\n");
    }

    printf("[IPC] App_IPC_Init done: Queue+Mutex+EventGroup+BinSem created.\r\n");
}
