  ...
  /* ====== 分支1：检查参数区里的 OTA 请求标志 ====== */
{
    ...
    {
        /* =============================================================
         *   分支1：参数区有有效 OTA 请求（D12 WiFi OTA 会写这个标志）
         *   策略：3 秒倒计时给用户按 KEY0 进串口 IAP 救砖
         *     - 3 秒内按了 KEY0 → 立刻跑 IAP_ProcessSerial（Ymodem 收写跳）
         *     - 3 秒没按 → 继续走分支2 KEY0 检查 + 分支3 跳 APP
         *     - ?? 坑点预警8：先 FlashParam_ClearOtaRequest() 再等 KEY0
         *       避免 IAP 失败后下次复位又进这里死循环
         * ============================================================= */

        /* 把 OTA 请求期望值打印出来，方便用户判断是不是 D12 已经写了正确信息 */
        printf("\r\n[BOOT] =================================================\r\n");
        ...
        /*  先清 OTA 请求标志（哪怕 IAP 失败也不会反复卡在这里） */
        FlashParam_ClearOtaRequest();
        printf("[BOOT] OTA_request flag pre-cleared (safe for fail-retries)\r\n");

        /* 3 秒倒计时等 KEY0：倒计时从 3 → 0，每秒减 1 */
        printf("[BOOT] Press KEY0 within 3 seconds to enter Serial IAP (Ymodem)...\r\n");
        int key_pressed = 0;
        uint32_t t_start = HAL_GetTick();
        for (int sec = 3; sec > 0; sec--) {
            printf("[BOOT]   %d...\r\n", sec);
            /* 每 1 秒期间，50ms 轮询一次 KEY0 */
            uint32_t t_sec_start = HAL_GetTick();
            while (HAL_GetTick() - t_sec_start < 1000) {
                if (HAL_GPIO_ReadPin(KEY0_GPIO_Port, KEY0_Pin) == GPIO_PIN_RESET) {
                    key_pressed = 1;
                    printf("[BOOT] KEY0 pressed at %d sec countdown! Starting IAP...\r\n", sec);
                    goto start_iap;   /* 跳出双层循环，直接跑 IAP */
                }
                HAL_Delay(50);
            }
        }
        (void)t_start;   /* 防 compiler warning（没用但为了可读性留了变量）*/

        /* 3 秒没按 KEY0：打印提示，继续往下走分支2/3 */
        if (!key_pressed) {
            printf("[BOOT] KEY0 not pressed within 3s. Continue to APP check.\r\n");
            printf("[BOOT] (Tip: WiFi OTA is TODO D12. Current D10 only supports KEY0+Serial IAP)\r\n");
        }

        /* 入口 A 跑到这里说明：要么用户 3 秒没按键（走分支2/3），要么已 goto start_iap 执行了 */
        goto end_branch1;   /* 跳过 start_iap 标签下面的代码 */

start_iap:
        /* ===== 跑真正的串口 IAP（Ymodem）===== */
        /* LED1 持续快闪表示"IAP 进行中"——IAP_ProcessSerial 内部成功时会复位，不会返回 */
        /* 如果返回说明失败了，继续跑分支2/3（看旧 APP 还能不能跳）*/
        HAL_GPIO_WritePin(LED1_GPIO_Port, LED1_Pin, GPIO_PIN_RESET);   /* LED1 亮，提示进入 IAP */
        (void)IAP_ProcessSerial();
        /* 如果 IAP 走到这里说明失败（写 Flash 错 / CRC 不匹配等）*/
        printf("[BOOT] IAP_ProcessSerial returned! APP flash may be corrupted.\r\n");
        printf("[BOOT] Attempting APP validity check... (if invalid, KEY0+Reset to retry)\r\n");
        HAL_GPIO_WritePin(LED1_GPIO_Port, LED1_Pin, GPIO_PIN_SET);   /* LED1 灭，提示失败 */

end_branch1:
        (void)key_pressed;   /* 防 unused variable warning */
    }
