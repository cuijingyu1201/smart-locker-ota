...
/* 编译期结构体大小检查：必须正好是 PARAM_FLASH_SIZE (4096 字节) */
_Static_assert(sizeof(flash_param_t) == 4096,
               "flash_param_t size MUST be exactly 4096 bytes (1 page) !");
_Static_assert(offsetof(flash_param_t, ota_new_fw_ver_major) == 0x68,
               "OTA target version offset MUST be 0x68");
_Static_assert(offsetof(flash_param_t, tail_marker) == 0xFF0,
               "tail marker offset MUST be 0xFF0");
