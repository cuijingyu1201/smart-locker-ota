 ...
 *   ?? 任何字段顺序/大小的修改，都必须两边工程同时改 flash_partition.h
 * ================================================================ */
typedef struct __attribute__((packed, aligned(4))) {
    ...
    uint16_t ota_new_fw_ver_major; /* 0x68: OTA 目标主版本 */
...
/* C99 静态断言：利用 typedef 负数数组会编译报错的特性 ，静态断言宏*/
#define GLUE(a, b)   a##b
#define STATIC_ASSERT_LINE(cond, msg, line) typedef char GLUE(static_assert_line, line) [(cond) ? 1 : -1]
#define STATIC_ASSERT(cond, msg) STATIC_ASSERT_LINE(cond, msg, __LINE__)

STATIC_ASSERT(sizeof(flash_param_t) == 4096, param_size_must_be_4096_bytes);
STATIC_ASSERT(offsetof(flash_param_t, ota_new_fw_ver_major) == 0x68,
              ota_target_version_offset_must_be_0x68);
STATIC_ASSERT(offsetof(flash_param_t, tail_marker) == 0xFF0,
              tail_marker_offset_must_be_0xFF0);
