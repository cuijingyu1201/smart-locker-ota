...
/* ================================================================
 *   CRC32 流式接口实现（D10 IAP 用）—— 复用本文件已有的 static const crc32_table[256]
 *   保证：1 份表、1 套算法，flash_param.c 和 iap.c 共享，不再各写一遍
 * ================================================================ */
...
{
    if (!ctx || !data) return;
    /* 直接用本文件顶部定义的 static const crc32_table（编译器已经知道它在哪了）*/
    uint32_t crc = *ctx;
    for (uint32_t i = 0; i < len; i++) {
        crc = (crc >> 8) ^ crc32_table[(crc ^ data[i]) & 0xFF];
    }
