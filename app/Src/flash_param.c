...
int FlashParam_InitOnBoot(void)
{
    ...
    CRC32_InitTable();
    ret = FlashParam_Load(&g_param_buf_internal);
    if (ret != 0) {
        uart_printf_mutex("[PARAM] Uninitialized, loading defaults...\r\n");
        ...
        uart_printf_mutex("[PARAM] Loaded: fw=%u.%u.%u build%u, boots=%lu, prev_ota=%lu\r\n",
                          ...
                          g_param_buf_internal.fw_ver_minor,
                          g_param_buf_internal.fw_ver_patch,
                          g_param_buf_internal.fw_build_num,
                          (unsigned long)g_param_buf_internal.boot_count,
                          (unsigned long)g_param_buf_internal.last_ota_result);
    }
    ...
    if (FlashParam_Save(&g_param_buf_internal) != 0) {
        uart_printf_mutex("[PARAM] Save FAIL!\r\n");
        return -1;
    }

    /* Read back and validate the full page so a partial write is caught now. */
    if (FlashParam_Load(&g_param_buf_internal) != 0) {
        uart_printf_mutex("[PARAM] Read-back verification FAIL!\r\n");
        return -1;
    }

    uart_printf_mutex("[PARAM] Saved ok. boot_count = %lu\r\n",
                      (unsigned long)g_param_buf_internal.boot_count);
    FlashParam_Print(&g_param_buf_internal);
    return 0;
}
...
{
    if (ver_major == NULL || ver_minor == NULL ||
        ver_patch == NULL || build_num == NULL) {
        return -1;
    }
    if (g_param_buf_internal.fw_ver_major == 0xFFFFU &&
        g_param_buf_internal.fw_ver_minor == 0xFFFFU &&
        g_param_buf_internal.fw_ver_patch == 0xFFFFU &&
        g_param_buf_internal.fw_build_num == 0xFFFFU) {
        return -1;
    }
    *ver_major = g_param_buf_internal.fw_ver_major;
    *ver_minor = g_param_buf_internal.fw_ver_minor;
    *ver_patch = g_param_buf_internal.fw_ver_patch;
    *build_num = g_param_buf_internal.fw_build_num;
    return 0;
}
...
int FlashParam_Save(const flash_param_t *p_in)
{
    ...
    uart_printf_mutex("[PARAM] Save start...\r\n");   /* ← 加这行 */

    if (p_in != &g_param_buf_internal) {
    ...
    uart_printf_mutex("[PARAM] CRC32 calc done: 0x%08X\r\n", g_param_buf_internal.struct_crc32);

    uart_printf_mutex("[PARAM] Erasing page @0x%08X...\r\n", PARAM_FLASH_START);
    if (FLASH_ErasePage(PARAM_FLASH_START) != HAL_OK) {
        uart_printf_mutex("[PARAM] Erase FAIL!\r\n");
        return -1;
    }
    uart_printf_mutex("[PARAM] Erase OK\r\n");

    uart_printf_mutex("[PARAM] Writing buffer...\r\n");
    if (FLASH_WriteBuf(PARAM_FLASH_START, (uint8_t *)&g_param_buf_internal, sizeof(flash_param_t)) != HAL_OK) {
        uart_printf_mutex("[PARAM] Write FAIL!\r\n");
        return -1;
    }
    uart_printf_mutex("[PARAM] Write OK\r\n");
                            ...
                            uint16_t ver_patch, uint16_t build_num)
{
    ...
    g_param_buf_internal.ota_new_fw_ver_major = ver_major;
    g_param_buf_internal.ota_new_fw_ver_minor = ver_minor;
    g_param_buf_internal.ota_new_fw_ver_patch = ver_patch;
    g_param_buf_internal.ota_new_fw_build_num = build_num;
...
int FlashParam_ClearOtaRequest(void)
{
    ...
    g_param_buf_internal.ota_new_fw_ver_major = 0xFFFFU;
    g_param_buf_internal.ota_new_fw_ver_minor = 0xFFFFU;
    g_param_buf_internal.ota_new_fw_ver_patch = 0xFFFFU;
    g_param_buf_internal.ota_new_fw_build_num = 0xFFFFU;
    return FlashParam_Save(&g_param_buf_internal);
}
...
{
    if (p == NULL) {
        uart_printf_mutex("[PARAM] NULL pointer!\r\n");
        return;
    }
    ...
    uart_printf_mutex("  fw_crc32        = 0x%08X\r\n", p->fw_crc32);
    uart_printf_mutex("  ota_req_magic   = 0x%08X (%c%c%c%c)\r\n", p->ota_request_magic,
