 ...
 * @version     V1.2 (D17 移除 SYSTEM/sys.h 耦合，改为直接 include main.h)
 * @date        2023-05-31
...
#include "stdlib.h"
#include "main.h"   /* D17 修改：去掉 "./SYSTEM/sys/sys.h"，通过 main.h 引入 stm32f1xx_hal.h + GPIO/RCC/FSMC 宏 */

/******************************************************************************************/
/* LCD RST/WR/RD/BL/CS/RS 引脚 定义
 * LCD_D0~D15,由于引脚太多,就不在这里单独定义了,直接在lcd_init里面修改.大家移植的时候,按
 * 照自己的接线,修改lcd_init函数里面D0~D15相关的IO即可.
 */

/* RESET 引脚 在系统复位后就一直拉高 这里就不做软件 RESET了 */
...
/* LCD_CS(需要根据LCD_FSMC_NEX来确认正确的IO口) 和 LCD_RS(需要根据LCD_FSMC_AX来确认正确的IO口) 引脚 定义 */
...
/* FSMC相关参数 定义
 * 注意: 我们默认是通过FSMC块1来驱动LCD, 块1有4个片选: FSMC_NE1~4
 *
 * 修改LCD_FSMC_NEX, 对应的LCD_CS_GPIO引脚设置也要改
 * 修改LCD_FSMC_AX , 对应的LCD_RS_GPIO引脚设置也要改
 */
...
typedef struct
{
    ...
    uint16_t id;        /* LCD ID */
    uint8_t dir;        /* 横屏还是竖屏设置，0表示竖屏，1表示横屏 */
...
/* LCD参数 */
extern _lcd_dev lcddev; /* 属性LCD重要参数 */
...
/* LCD_BASE的详细计算方法:
 * 这里我们使用FSMC的块1(BANK1)来驱动TFTLCD液晶屏(MCU屏), 块1地址范围总大小为256MB,均分成4块:
 ...
 *
 * 我们需要根据硬件连接方式选择合适的片选(即LCD_CS)和地址线(即LCD_RS)
 * 精英F103开发板使用FSMC_NE4接LCD_CS, FSMC_A10接LCD_RS ,16位数据宽度,计算方法如下:
 *     FSMC_NE4的基地址为: 0X6C00 0000;     NEx的基地址为(x=1/2/3/4): 0X6000 0000 + (0X400 0000 * (x - 1))
 *     FSMC_A10对应的值地址: 2^10 * 2 = 0X800;    FSMC_Ay对应的地址为(y = 0 ~ 25): 2^y * 2
 *
 * LCD->LCD_REG,对应LCD_RS = 0(LCD寄存器); LCD->LCD_RAM,对应LCD_RS = 1(LCD数据)
 * 所以 LCD->LCD_RAM的地址为:  0X6C00 0000 + 2^10 * 2 = 0X6C00 0800
 *    LCD->LCD_REG的地址可以为 LCD->LCD_RAM之前的任意地址.
 * 所以我们这里使用结构体来映射LCD_REG 和 LCD_RAM(REG在前,RAM在后,都是16位数据宽度)
 * 即 结构体的基地址(LCD_BASE) = LCD_RAM - 2 = 0X6C00 0800 -2
 *
 * 更加通用的计算公式为((片选脚FSMC_NEx)x=1/2/3/4, (RS脚接的地址线FSMC_Ay)y=0~25):
 *          LCD_BASE = (0X6000 0000 + (0X4000 0000 * (x - 1))) | (2^y * 2 -2)
 *          等效成(使用移位运算):
 *          LCD_BASE = (0X6000 0000 + (0X4000 0000 * (x - 1))) | ((1 << y) * 2 -2)
 */
...
void lcd_display_dir(uint8_t dir);          /* 设置LCD屏幕显示方向 */
...
void lcd_fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint32_t color);          /* 单色填充区域(32位颜色,兼容LTDC) */
