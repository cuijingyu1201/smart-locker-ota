...
D13：OTA 容错测试与安全加固（CRC32 失败不写 flag + 擦除后读回验证 + 升级中断电自动进串口 IAP + last_ota_result 中断标志）（2026-08-28）
...
## 坑2：APP 端不清 last_ota_result → IAP 刷完后下次 Reset 又进 IAP 死循环
...
- **排查过程**：
  ...
  2. 读 APP 端 `FlashParam_InitOnBoot` → 发现只改了 boot_count，没清 last_ota_result
  3. 确认：APP 正常启动 = 固件没问题，应该把 last_ota_result 清成 0
...
- **代码改动量**：Bootloader 新增约 25 行（擦除验证 + last_ota_result 读写 + 启动检查），APP 新增 1 行（`last_ota_result=0`），共约 26 行防御性代码。
